...
function crash() {
  ...
  try {
    ...
    localStorage.setItem('cloneWarsBest', String(bestScore));
  } catch (error) {}
...
function frame(timestamp) {
  const seconds = Math.min(0.05, lastTime ? (timestamp - lastTime) / 1000 : 0);
  lastTime = timestamp;
  ...
  if (state === 'playing') {
    ...
    for (const pipe of pipes) {
      ...
      const overlapsPipe = birdRight > pipe.x && birdLeft < pipe.x + CONFIG.pipeWidth;
      if (overlapsPipe && (birdTop < pipe.gapTop || birdBottom > pipe.gapBottom)) {
        crash();
        break;
      }
    }
    pipes = pipes.filter((pipe) => pipe.x + CONFIG.pipeWidth > 0);
    if (state === 'playing' && (y + half >= CONFIG.canvasHeight - CONFIG.groundHeight || y - half <= 0)) crash();
  ...
  if (state === 'playing') {
    ...
    ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 60);
    ctx.fillText(String(score), CONFIG.canvasWidth / 2, 60);
    ctx.restore();
  }
